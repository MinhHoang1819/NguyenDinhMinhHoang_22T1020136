    ...
    public static class WebUserExtensions
    {
        ...
        public static WebUserData? GetUserData(this ClaimsPrincipal principal)
        {
            try
            {
                ...
                userData.UserId = principal.FindFirstValue(nameof(WebUserData.UserId))
                    ?? principal.FindFirstValue(ClaimTypes.NameIdentifier);
                userData.UserName = principal.FindFirstValue(nameof(WebUserData.UserName))
        ...
        /// <summary>
        /// Đọc thông tin user đã lưu trong cookie (claims) từ HttpContext.
        /// </summary>
        public static WebUserData? GetUserData(this HttpContext httpContext)
        {
            return httpContext?.User.GetUserData();
        }
    }
}