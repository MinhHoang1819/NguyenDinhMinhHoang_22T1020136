...
namespace SV22T1020136.DataLayers.SQLServer
{
    ...
    public class CustomerRepository : BaseSQLDAL, ICustomerRepository
    {
        ...
        {
            if (string.IsNullOrWhiteSpace(storedHashOrPlain) || plainPassword == null)
                return false;

            var md5 = CryptHelper.ToMD5(plainPassword);
        ...
        public async Task<int> AddAsync(Customer data)
        {
            using (var connection = GetConnection())
            {
                ...
                var parameters = new
                {
                    ...
                    data.Phone,
                    data.Email,
                    Password = CryptHelper.ToMD5(data.Password ?? ""),
                    data.IsLocked
                };
        ...
        public async Task<bool> ChangePasswordAsync(int id, string password)
        {
            using (var connection = GetConnection())
            {
                ...
                var parameters = new { Password = CryptHelper.ToMD5(password ?? ""), CustomerID = id };
                var rowsAffected = await connection.ExecuteAsync(sql, parameters);
                return rowsAffected > 0;
            }
        }
        ...
        public async Task<Customer?> AuthorizeAsync(string email, string password)
        {
            ...
            {
                await connection.OpenAsync();
                var sql = @"SELECT TOP 1 * FROM Customers WHERE Email = @Email AND IsLocked = 0";
                var customer = await connection.QueryFirstOrDefaultAsync<Customer>(sql, new { Email = email });
                return PasswordMatchesStored(customer?.Password, password) ? customer : null;
            }
        }
    }
}
